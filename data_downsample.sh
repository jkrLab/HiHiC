...
# examples
# bash data_download_downsample.sh https://ftp.ncbi.nlm.nih.gov/geo/samples/GSM1551nnn/GSM1551550/suppl/GSM1551550_HIC001_merged_nodups.txt.gz GSM1551550_HIC001 hg19 16 ./juicer_tools.jar KR 10000 data
# bash data_download_downsample.sh https://ftp.ncbi.nlm.nih.gov/geo/samples/GSM1551nnn/GSM1551620/suppl/GSM1551620_HIC071_merged_nodups.txt.gz GSM1551620_HIC071 hg19 16 ./juicer_tools.jar KR 10000 data_K562
# bash data_download_downsample.sh https://ftp.ncbi.nlm.nih.gov/geo/samples/GSM1551nnn/GSM1551643/suppl/GSM1551643_HIC098_merged_nodups.txt.gz GSM1551643_HIC098 mm9.chrom.sizes 16 ./juicer_tools.jar KR 10000 data_ch12-lx

# 필요한 매개변수 체크
if [ "$#" -ne 8 ]; then
  echo "Usage: $0 <download_url> <file_name> <ref_genome> <downsample_ratio> <juicertools> <normalization> <resolution> <saved_in>"
  exit 1
fi
...
sample_size=$(( total_lines / downsample_ratio ))
...
# 결과를 저장할 폴더 생성​​
mkdir ${path}/${saved_in}_downsampled_${downsample_ratio}
# 전체 크로모좀에 대하여 반복문으로 명령 실행
for ((chrom=1; chrom<end; chrom++))
do
    java -jar ${juicertools} dump observed ${normalization} ./${file_name}_ds_${downsample_ratio}.hic ${chrom} ${chrom} BP ${resolution} ${path}/${saved_in}_downsampled_${downsample_ratio}/chr${chrom}_${resolution}.txt
done
