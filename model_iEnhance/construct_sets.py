...
optional.add_argument('-n', '--prefix', dest='prefix', type=str, required=False, help='Prefix of the generated files')
...
np.savez(os.path.join(train_dir, f"{args.prefix}_{args.max_value}.npz"),target = hrtraind,data = lrtraind)
...
np.savez(os.path.join(valid_dir, f"{args.prefix}_{args.max_value}.npz"),target = hrvalid,data = lrvalid)
...
for c in testlist:
    ...
    dataslr = np.load(deal_fd + f"/{args.prefix}_{args.max_value}_chr{c}.npz")['data']
    
    # whole_hrmat[c] = datashr
    whole_lrmat[c] = dataslr

np.savez(os.path.join(test_dir, f"{args.prefix}_{args.max_value}.npz"), **whole_lrmat) ######################
###################################################################################################