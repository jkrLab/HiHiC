...
for model in "${models[@]}"; do
    ...
    if [ ${model} == "SRHiC" ]; then # input type .npy
    ...
    elif [ ${model} == "DFHiC" ]; then # gpu 사용시 out of memory 
        for read in "${reads[@]}"; do
            bash model_prediction.sh -m ${model} -c ${checkpoint} -b 16 -g -1 -r ${read} -i ./data_${model}/for_enhancement_${model}_ch12-lx_${read}_10000.npz -o ./predicted_ch12-lx
            bash model_prediction.sh -m ${model} -c ${checkpoint} -b 16 -g -1 -r ${read} -i ./data_${model}/for_enhancement_${model}_GM12878_${read}_10000.npz -o ./predicted_GM12878
            bash model_prediction.sh -m ${model} -c ${checkpoint} -b 16 -g -1 -r ${read} -i ./data_${model}/for_enhancement_${model}_K562_${read}_10000.npz -o ./predicted_K562
        done
        ...
        for read in "${reads[@]}"; do
            bash model_prediction.sh -m ${model} -c ${checkpoint} -b 16 -g 0 -r ${read} -i ./data_${model}/for_enhancement_${model}_ch12-lx_${read}_10000.npz -o ./predicted_${model}
            bash model_prediction.sh -m ${model} -c ${checkpoint} -b 16 -g 0 -r ${read} -i ./data_${model}/for_enhancement_${model}_GM12878_${read}_10000.npz -o ./predicted_${model}
            bash model_prediction.sh -m ${model} -c ${checkpoint} -b 16 -g 0 -r ${read} -i ./data_${model}/for_enhancement_${model}_K562_${read}_10000.npz -o ./predicted_${model}
        done
