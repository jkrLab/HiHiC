...
while getopts ":i:d:r:b:m:g:o:s:t:v:p:w:" flag; do
    case $flag in
...
if [ -z "${input_data_dir}" ] || [ -z "${input_downsample_dir}" ] || [ -z "${bin_size}" ] || [ -z "${model}" ] || [ -z "${ref_genome}" ] || [ -z "${saved_in}" ]|| [ -z "${train_set}" ] || [ -z "${prediction_set}" ]; then
    echo "Usage: $0 -i <input_data_path> -d <input_downsample_path> -b <resolution> -m <model_name> -g <ref_genome> -o <saved_in> -s <max_value> -t <train_set_chromosome> -v <valid_set_chromosome> -p <prediction_set_chromosome> [-w <workers>]" >&2
    exit 1
fi
...
if [ "${model}" = "iEnhance" ]; then
    ...
    python3 -u data_generate_for_training.py -i "${input_data_dir}" -d "${input_downsample_dir}" -b "${bin_size}" -m "${model}" -g "${ref_genome}" -o "${saved_in}/data_${model}/" -s "${max_value}" -t "${train_set}" -v "${valid_set}" -p "${prediction_set}" -w "${workers:-$(nproc)}"
fi
